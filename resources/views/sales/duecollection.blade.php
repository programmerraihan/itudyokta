            ...
            <div class="row">
                ...
                <div class="col-xl-12">
                    <div class="card shadow">
                        ...
                        <div class="card-body">
                            <form action="{{route("sales.dueCollected", ["sale" => $sale->id])}}" method="post">
                                @csrf @method('POST') 
                                {{-- total quantity --}}
                                <input type="hidden" name="quantity" value="{{$sale->quantity}}" />
                                <input type="hidden" name="amount" value="{{$sale->amount}}" />
                                <input type="hidden" name="discount_amount" value="{{$sale->discount_amount}}" />
                                <input type="hidden" name="grand_total" value="{{$sale->grand_total}}" />
                                <div class="form-row">
                                    <div class="col-md-6 col-sm-12 form-group">
                                        <label for="date">Date*</label>
                                        <input type="date" name="date" value="{{$sale->date}}" class="form-control @error("date") is-invalid @enderror " readonly required/>
                                        @error('date')
                                    ...
                                    <div class="col-md-6 col-sm-12 form-group">
                                        ...
                                        <input type="text" name="reference_no" value="{{$sale->reference_no}}" class="form-control @error("reference_no") is-invalid @enderror " required readonly/>
                                        @error('reference_no')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="col-md-6 col-sm-12 form-group">
                                        ...
                                        <input type="text" name="customer" value="{{$sale->customer}}" id="customer" placeholder="Customer" class="form-control @error("customer") is-invalid @enderror " required readonly/>
                                        @error('customer')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    <div class="col-md-6 col-sm-12 form-group">
                                        ...
                                        <input type="tel" name="phone" value="{{$sale->phone}}" id="phone" placeholder="Phone" class="form-control @error("phone") is-invalid @enderror " readonly/>
                                        @error('phone')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="note" id="note" cols="30" rows="2" class="form-control" placeholder="Note">{{$sale->note}}</textarea>
                                </div>
                                ...
                                <table class="table table-sm table-bordered" id="product_list">
                                    ...
                                    <tbody>
                                        @forelse ($sale->saleItems as $saleItem)
                                        <tr>
                                            <input type="hidden" name="product_id[]" value="{{$saleItem->product_id}}" />
                                            <input type="hidden" name="unit_id[]" value="{{$saleItem->unit_id}}" />
                                            <td><input type="text" readonly name="product_name[]" value="{{$saleItem->product->name}}" class="form-control" /></td>
                                            <td><input type="text" readonly name="unit_name[]" value="{{$saleItem->unit->name}}" class="form-control" /></td>
                                            <td><input type="number" min="0" name="unit_quantity[]" value="{{$saleItem->quantity}}" class="form-control quantity" /></td>
                                            <td><input type="number" min="0" name="unit_price[]" value="{{$saleItem->unit_price}}" class="form-control price" /></td>
                                            <td><input type="number" min="0" name="unit_discount[]" value="{{$saleItem->discount}}" class="form-control discount" /></td>
                                           <td><input type="number" min="0" name="unit_total[]" value="{{$saleItem->amount}}" class="form-control total" readonly /></td>
                                           <td></td>
                                        </tr>
                                        @empty                                            
                                        @endforelse
                                    ...
                                    <tfoot class="bg-light">
                                        <tr>
                                            ...
                                            <th id="total_qty">{{$sale->quantity}}</th>
                                            <th>&nbsp;</th>
                                            <th id="total_discount">{{$sale->discount_amount}}</th>
                                            <th id="total_amount">{{$sale->amount}}</th>
                                            <td>&nbsp;</td>
                                        </tr>
                                    </tfoot>
                                </table>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="col-md-4 col-sm-12 form-group">
                                        ...
                                        <input type="number" name="net_amount" id="net_amount" placeholder="Net Amount" value="{{$sale->grand_total}}" class="form-control @error("net_amount") is-invalid @enderror " readonly />
                                        @error('net_amount')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="col-md-4 col-sm-12 form-group">
                                        ...
                                        <input type="number" value="{{$sale->paid_amount}}" name="paid_amount" id="paid_amount" placeholder="Paid Amount" class="form-control @error("paid_amount") is-invalid @enderror "/>
                                        @error('paid_amount')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
